...
//
// CHAOS-58 made the seam CONTEXT-AWARE. It used to be `net.LookupHost`, whose
...
type hostIPCache struct {
	...
	// requests to one host (CHAOS-58). During a resolver brownout that is one
...
const (
	...
	//
	// Before CHAOS-58 an expired entry was discarded outright, which made the
...
func (c *hostIPCache) lookup(host string, now time.Time) (ip net.IP, state hostIPState, refreshing bool) {
	...
	case e.ip != nil && now.Before(e.expiry.Add(hostIPCacheStaleMax)):
		...
		// pre-CHAOS-58 dark window, reintroduced by the mechanism meant to close
...
// CHAOS-58 a total resolution outage on this path was counted nowhere, logged
