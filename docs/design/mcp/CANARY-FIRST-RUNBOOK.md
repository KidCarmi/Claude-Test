...
| Canary architecture — scope gate (`ValidateScope`) | **PRESENT** — forbids percentage/group/wildcard and caps server & tenant at 1. `MaxCanaryTools`/`MaxCanaryPrincipals` remain **2** BY DESIGN: they bound the Canary ARCHITECTURE a later graduation phase may use, and `principalCount` still sums Principals+Clients+Agents for that broader contract |
| First-experiment exact-scope gate (`ValidateFirstCanaryScope`) | **PRESENT** — the one-tool/one-synthetic-principal shape is now a MACHINE gate, not an external prerequisite (review §10/§25a, blocker 5 CLOSED). A separate predicate layered on `ValidateScope`, evaluated on the SIGNED activation scope in `evaluateActivationOnFacts`: exactly 1 tenant + 1 server + 1 fully-pinned tool ON that server + 1 explicitly named `Principals` entry; `Clients`/`Agents`/`Groups`/`Environments`/`ToolFingerprints`/all four `Exclude*` empty; `Percent` 0; no duplicate, empty, over-long or glob-shaped identifier. Identity is counted on `Principals` ALONE, so the `principalCount` aggregate can never satisfy it, and a duplicate in the signed object is refused rather than deduplicated. A wider signed scope makes the activation preflight report `canary_scope_not_exact_first_canary` and `Ready:false`, so it cannot activate at all |
