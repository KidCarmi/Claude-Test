...
func stubResolver(addrs []string, err error) (calls *atomic.Int64, restore func()) {
	...
	return &counter, func() {
		// CHAOS-59 made the stale path refresh ASYNCHRONOUSLY, so a background
...
// TestResolveHost_TTLExpiry pins the CHAOS-59 stale-while-revalidate contract:
...
//
// The pre-CHAOS-59 contract was a synchronous re-resolve on expiry, which is
