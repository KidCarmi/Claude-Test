...
func stubResolver(addrs []string, err error) (calls *atomic.Int64, restore func()) {
	...
	return &counter, func() {
		// CHAOS-58 made the stale path refresh ASYNCHRONOUSLY, so a background
...
// TestResolveHost_TTLExpiry pins the CHAOS-58 stale-while-revalidate contract:
...
//
// The pre-CHAOS-58 contract was a synchronous re-resolve on expiry, which is
