...
// dns_resolve_chaos_test.go — CHAOS-59 gates.
//
...
// TestChaos59_WedgedResolverDoesNotBlockTheRequestPathForever pins the
...
func TestChaos59_WedgedResolverDoesNotBlockTheRequestPathForever(t *testing.T) {
	...
	go func() {
		start := time.Now()
		_ = resolveHost("wedged.chaos58.invalid")
...
// TestChaos59_TimeoutIsCountedAndClassified: a deadline overrun must be
// visible. Pre-fix nothing on this path counted, logged or alerted a failure.
func TestChaos59_TimeoutIsCountedAndClassified(t *testing.T) {
	release := make(chan struct{})
	defer close(release)
	blockingResolver(t, release, nil, nil)

	_ = resolveHost("timeout-counted.chaos58.invalid")
...
// TestChaos59_ConcurrentMissesForOneHostResolveOnce pins the single-flight.
//
...
// failing.
func TestChaos59_ConcurrentMissesForOneHostResolveOnce(t *testing.T) {
	release := make(chan struct{})
	...
	for i := 0; i < goroutines; i++ {
		...
		go func(i int) {
			defer wg.Done()
			started <- struct{}{}
			results[i] = resolveHost("herd.chaos58.invalid")
		}(i)
	}
	for i := 0; i < goroutines; i++ {
		<-started
...
// TestChaos59_FollowersAreReleasedWhenTheLeaderSheds: a leader that sheds must
...
func TestChaos59_FollowersAreReleasedWhenTheLeaderSheds(t *testing.T) {
	release := make(chan struct{})
	defer close(release)
	blockingResolver(t, release, nil, nil)
	...
	go func() {
		...
		for i := 0; i < 50; i++ {
			wg.Add(1)
			go func() { defer wg.Done(); _ = resolveHost("shed-followers.chaos58.invalid") }()
		}
		wg.Wait()
	}()
...
// TestChaos59_ResolverOutageDoesNotTakeGeoRulesDark is the security half.
//
...
// every host at once.
func TestChaos59_ResolverOutageDoesNotTakeGeoRulesDark(t *testing.T) {
	calls, restore := stubResolver([]string{"203.0.113.58"}, nil)
	defer restore()

	warm := resolveHost("outage.chaos58.invalid")
	if warm == nil {
	...
	lookupHostFn = func(_ context.Context, _ string) ([]string, error) {
		return nil, &net.DNSError{Err: "server misbehaving", Name: "outage.chaos58.invalid"}
	}
	defer func() {
		drainInflightResolutions()
		lookupHostFn = origFn
	}()
	expireEntry("outage.chaos58.invalid", time.Second)

	got := resolveHost("outage.chaos58.invalid")
...
// TestChaos59_FailedRefreshDoesNotDestroyTheStaleAnswer proves the background
...
func TestChaos59_FailedRefreshDoesNotDestroyTheStaleAnswer(t *testing.T) {
	calls, restore := stubResolver([]string{"203.0.113.59"}, nil)
	defer restore()

	warm := resolveHost("refresh-fail.chaos58.invalid")
	if warm == nil {
		t.Fatal("warm-up resolution failed")
	}
	waitForResolverCalls(t, calls, 1)
	...
	origFn := lookupHostFn
	lookupHostFn = func(_ context.Context, _ string) ([]string, error) {
		return nil, &net.DNSError{Err: "server misbehaving", Name: "refresh-fail.chaos58.invalid"}
	}
	defer func() {
		drainInflightResolutions()
		lookupHostFn = origFn
	...
	expireEntry("refresh-fail.chaos58.invalid", time.Second)
	if got := resolveHost("refresh-fail.chaos58.invalid"); got == nil {
	...
	if got := resolveHost("refresh-fail.chaos58.invalid"); got == nil || !got.Equal(warm) {
...
// TestChaos59_StaleServesDoNotStackRefreshes pins at most ONE refresh per host,
...
func TestChaos59_StaleServesDoNotStackRefreshes(t *testing.T) {
	...
	if resolveHost("stale-stack.chaos58.invalid") == nil {
	...
	expireEntry("stale-stack.chaos58.invalid", time.Second)
	...
	for i := 0; i < 100; i++ {
		...
		go func() {
			defer wg.Done()
			if ip := resolveHost("stale-stack.chaos58.invalid"); ip == nil {
...
// TestChaos59_ExpiredNegativeEntryIsNotStaleServed pins that stale serving is
...
func TestChaos59_ExpiredNegativeEntryIsNotStaleServed(t *testing.T) {
	calls, restore := stubResolver(nil, &net.DNSError{Err: "server misbehaving", Name: "neg-stale.chaos58.invalid"})
	defer restore()

	if ip := resolveHost("neg-stale.chaos58.invalid"); ip != nil {
	...
	expireEntry("neg-stale.chaos58.invalid", time.Second)

	got := resolveHost("neg-stale.chaos58.invalid")
	if got == nil {
...
// TestChaos59_ExpiredNegativeStillReResolvesWhenTheResolverPoolIsSaturated is
// the load half of the same finding: the repair must not depend on the async
// refresh, which sheds when the pool is full.
func TestChaos59_ExpiredNegativeStillReResolvesWhenTheResolverPoolIsSaturated(t *testing.T) {
	calls, restore := stubResolver(nil, &net.DNSError{Err: "server misbehaving", Name: "neg-shed.chaos58.invalid"})
	defer restore()

	if ip := resolveHost("neg-shed.chaos58.invalid"); ip != nil {
		t.Fatalf("setup: failed resolution returned %v, want nil", ip)
	}
	waitForResolverCalls(t, calls, 1)
	drainInflightResolutions()
	expireEntry("neg-shed.chaos58.invalid", time.Second)
	...
	if got := resolveHost("neg-shed.chaos58.invalid"); got == nil || !got.Equal(net.ParseIP("203.0.113.98")) {
...
// TestChaos59_DistinctHostFloodIsShedNotQueued proves a herd spread across many
...
func TestChaos59_DistinctHostFloodIsShedNotQueued(t *testing.T) {
	release := make(chan struct{})
	defer close(release)
	blockingResolver(t, release, nil, nil)
...
func floodHost(i int) string {
	return "flood-" + strings.Repeat("x", i%3) + itoaSmall(i) + ".chaos58.invalid"
}
...
// TestChaos59_ShedResultIsNotCached: saturation is a fact about THIS node's
...
func TestChaos59_ShedResultIsNotCached(t *testing.T) {
	release := make(chan struct{})
	defer close(release)
	blockingResolver(t, release, nil, nil)
	restore := shrinkResolverPool(t, 0)
	...
	if ip := resolveHost("shed-nocache.chaos58.invalid"); ip != nil {
		t.Fatalf("shed resolution returned %v, want nil", ip)
	}
	resolvedHostCache.mu.RLock()
	_, cached := resolvedHostCache.entries["shed-nocache.chaos58.invalid"]
...
// TestChaos59_NXDOMAINNeverDrivesDegradation keeps an authoritative "no such
...
// the page on demand.
func TestChaos59_NXDOMAINNeverDrivesDegradation(t *testing.T) {
	resetDNSResolveHealthForTest()
...
// TestChaos59_DegradationIsADurationAndAlertsOnce pins the threshold as a
// DURATION and the page as fire-once per episode.
func TestChaos59_DegradationIsADurationAndAlertsOnce(t *testing.T) {
	resetDNSResolveHealthForTest()
	defer resetDNSResolveHealthForTest()
...
// TestChaos59_RecoveryIsOnObservedEvidence requires an observed successful
...
func TestChaos59_RecoveryIsOnObservedEvidence(t *testing.T) {
	resetDNSResolveHealthForTest()
	defer resetDNSResolveHealthForTest()
...
// TestChaos59_PrivateOnlyAnswerIsNotAResolverFault treats an answer carrying only
...
func TestChaos59_PrivateOnlyAnswerIsNotAResolverFault(t *testing.T) {
	resetDNSResolveHealthForTest()
	defer resetDNSResolveHealthForTest()
...
// TestChaos59_AlertDetailIsBounded pins the dedup-key contract on BOTH
...
// alerts (WK-12/RS-5).
func TestChaos59_AlertDetailIsBounded(t *testing.T) {
...
func TestChaos59_ClassifyDNSFailureClasses(t *testing.T) {
	cases := []struct {
		name string
		err  error
		want string
...
// TestChaos59_NXDOMAINAtTheDeadlineBoundaryStaysNXDOMAIN keeps an authoritative
...
func TestChaos59_NXDOMAINAtTheDeadlineBoundaryStaysNXDOMAIN(t *testing.T) {
...
// TestChaos59_ContractRowReportsWarnNotFail pins the degraded contract row at
...
// serving.
func TestChaos59_ContractRowReportsWarnNotFail(t *testing.T) {
	resetDNSResolveHealthForTest()
	defer resetDNSResolveHealthForTest()
...
// TestChaos59_ContractRowNeverCarriesAHostname: /api/diagnostics is a
...
func TestChaos59_ContractRowNeverCarriesAHostname(t *testing.T) {
	resetDNSResolveHealthForTest()
	defer resetDNSResolveHealthForTest()
...
func TestChaos59_Control_HealthyResolutionStillWorks(t *testing.T) {
	calls, restore := stubResolver([]string{"192.0.2.1", "203.0.113.77"}, nil)
	defer restore()

	got := resolveHost("healthy.chaos58.invalid")
	if got == nil {
...
func TestChaos59_Control_UnresolvableHostStillFailsAndIsCounted(t *testing.T) {
	calls, restore := stubResolver(nil, &net.DNSError{Err: "no such host", IsNotFound: true})
	defer restore()

	if ip := resolveHost("nxdomain.chaos58.invalid"); ip != nil {
...
// TestChaos59_StaleServeIsCounted: the stale-served counter is the leading
...
func TestChaos59_StaleServeIsCounted(t *testing.T) {
	calls, restore := stubResolver([]string{"203.0.113.88"}, nil)
	defer restore()

	if resolveHost("stale-counted.chaos58.invalid") == nil {
		t.Fatal("warm-up failed")
	}
	waitForResolverCalls(t, calls, 1)
	expireEntry("stale-counted.chaos58.invalid", time.Second)
	_ = resolveHost("stale-counted.chaos58.invalid")
