...
func (g *orderingGate) AdmitSideEffect(LiveGateInput) LiveGateDecision {
	...
	// A metered admission MUST name its reservation and activation: openAttempt
	// fails closed with ReasonEventEvidenceMissing when a non-nil gate admits with
	// a zero ReservationID or ActivationGeneration ("metered execution without
	// reservation identity"). This fake predates that contract, so it supplies
	// stable non-zero values; without them the request is refused before the
	// boundary assertions below ever run, and the suite proves nothing.
	return LiveGateDecision{
		Admit:                true,
		Revalidate:           g.revalidate,
		ReservationID:        "ordering-gate-reservation",
...
// AdmitAuxiliary satisfies the SEC-MCP-AUX-1 half of LiveExecutionGate (#1319). These tests
// exercise the SIDE-EFFECT boundary only, so auxiliary traffic is admitted with the same
// Release accounting and, deliberately, NO reservation or generation: an auxiliary call
// consumes no budget slot and names no attempt, and openAttempt never reaches its
// reservation-identity check for a non-metered method.
func (g *orderingGate) AdmitAuxiliary(LiveGateInput) LiveGateDecision {
	return LiveGateDecision{
		Admit: true,
		Release: func() {
			g.mu.Lock()
