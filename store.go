...
// timeSeries deliberately keeps a plain mutex, and the per-request increment
// stays INSIDE it. Two alternatives have now been measured and rejected:
//
//   - RLock + atomic increment (2026-07): FLAT (~154 ns/op both ways).
//
//   - Fully lock-free atomic increments, no mutex on the record path
//     (2026-09): WORSE under concurrency. Medians of
//     BenchmarkTSRecordResultParallel on a 4-core Xeon (mutex n=5,
//     lock-free n=8 isolated) —
//
//     GOMAXPROCS  │    1    │    2    │    4    │
//     plain mutex │ 69.6 ns │ 94.4 ns │ 151.2ns │
//     lock-free   │ 63.9 ns │ 260.3ns │ 186.4ns │
//
// The reason is the same in both cases and is the thing to remember before
// trying a third time: EVERY request increments the SAME current-minute
// bucket, so the shared cache line — not the lock — is the bound. Removing the
// mutex does not remove the contended line; it removes the batching. A mutex
// holder mutates the line while it owns it exclusively, whereas N cores doing
// atomic read-modify-writes on one address ping-pong that line on every
// operation, which is why the lock-free form got ~2.8x worse at two cores.
//
// Sharding the ring WOULD remove the shared line, and was prototyped
// (16 shards, both mutex and atomic within the shard). It did not pay on this
// hardware: there is no natural shard key on this path, so selection costs a
// rand.Uint64 at ~8-17 ns/op, which ate the gain — 85-97 ns/op sharded+mutex
// and 54-81 sharded+atomic at four cores, against 151 for the plain mutex
// measured in the same process. Worth revisiting only on much wider hardware,
// and only with a cheaper shard key than a PRNG call.
